...
from template_matching_helpers import match_symbol as match_symbol_against_templates
...
def match_symbol(symbol_image: np.ndarray, templates_dir):
    """Compatibility wrapper around the shared template matcher."""
    match = match_symbol_against_templates(symbol_image, templates_dir)
    return match["best_label"], match["best_score"]
...
def main() -> None:
    ...
    rank_match = match_symbol_against_templates(rank_gray, rank_templates_dir)

    print("\nMatching suit templates:")
    suit_match = match_symbol_against_templates(suit_gray, suit_templates_dir)
